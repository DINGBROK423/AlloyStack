    ...
    if #[cfg(feature = "ramdisk")] {
        ...
        impl DriverProbe for RamDiskDriver {
            fn probe_global() -> Option<AxDeviceEnum> {
                ...
                println!("使用特性ramdisk。");
                println!("RamDiskDriver::probe_global: IMAGE_PATH = {:?}", unsafe { &IMAGE_PATH });
                let size = std::fs::metadata(unsafe { &IMAGE_PATH }).unwrap().len() as usize;
                let ramdisk = driver_block::ramdisk::RamDisk::new(size);
                // let ramdisk = driver_block::ramdisk::RamDisk::new(unsafe { &IMAGE_PATH });
                println!("RamDiskDriver::probe_global: RamDisk created successfully");
